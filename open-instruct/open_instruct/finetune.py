...
def parse_args():
    ...
    parser.add_argument(
        "--val_file", type=str, default="./data/val_dataset_code_alpaca.jsonl", help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--model_name_or_path",
        type=str,
    ...
    parser.add_argument(
        "--data_ratio", type=float, default=10, help="Ratio of total training steps used for training data (full:10.0)."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.03,
        help='select how many parameters to update in each iteration (0.03 means 3% parameters update)',
    )
    parser.add_argument(
        '--clip_grad_norm',
        type=float,
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        print("before cccccccccccccccc: ",raw_datasets["train"].column_names)
        print("len train data: ", len(raw_datasets["train"]))
        if "pmc" in args.train_file:
            raw_datasets["train"] = raw_datasets["train"].select(range(int(51200*args.data_ratio)))
        else:
            raw_datasets["train"] = raw_datasets["train"].select(range(int(39500*args.data_ratio)))
        raw_datasets["val"] = raw_datasets["val"]
    ...
    if args.use_lora:
        lora_list = ["q_proj", "o_proj", "v_proj", "k_proj", "gate_proj", "up_proj", "down_proj"]
        if args.use_qlora:
    ...
    print("use MoFO", args.use_AdamW_MoFO)
    if args.use_qlora:
    ...
    elif args.use_AdamW_MoFO:
        print("MoFO's fraction: ", args.MoFO_fraction)
    ...
    #    initial_parameters_copy = [p.detach().cpu().clone() for p in model.parameters()]
    #    print(initial_parameters_copy)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                if "gemma" in args.model_name_or_path or "Mistral" in args.model_name_or_path:
                    accelerator.wait_for_everyone()
            ...
            if accelerator.sync_gradients:
                ...
                if completed_steps%200==0 or completed_steps == args.max_train_steps:
                    predict_val_list =[]
                    label_val_list= []
                    with torch.no_grad():
                        index_val = 0
                        ...
                        for step_val, batch in enumerate(val_dataloader):
                            with accelerator.accumulate(model):
                                ...
                                index_val +=1
                        final_val_loss = total_val_loss/index_val
                if args.logging_steps and completed_steps % args.logging_steps == 0:
                    predict_list =[]
                    label_list= []
                    
                    avg_loss = accelerator.gather(total_loss).mean().item() / args.gradient_accumulation_steps / args.logging_steps
                    logger.info(f"  Step: {completed_steps}, LR: {lr_scheduler.get_last_lr()[0]}, Loss: {avg_loss} training acc: {real_train_acc}, val loss: {final_val_loss}")
                    if args.with_tracking:
                        accelerator.log(
                            {
                                "learning_rate": lr_scheduler.get_last_lr()[0],
                                "train_loss": avg_loss,
                                "train accuracy": real_train_acc,
                ...
                if isinstance(checkpointing_steps, int):
                    if completed_steps % checkpointing_steps == 0 and completed_steps!= 600:
